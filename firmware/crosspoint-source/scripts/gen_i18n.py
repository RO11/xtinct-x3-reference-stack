...
def load_translations(
    ...
    # English is both the source language and every missing translation's
    # fallback.  A blank or literal key here therefore corrupts every locale
    # and can silently ship labels such as "STR_PHONE_SYNC" to the device.
    for key in string_keys:
        value = english_data[key]
        if not value.strip():
            raise ValueError(f"English value for '{key}' must not be blank")
        if value.strip() == key:
            raise ValueError(f"English value for '{key}' must not equal its translation key")
