...
void runLanguageEval(const char* langName, const char* primaryTag, const char* resourceFile, double minF1Percent) {
  ...
  const char* runtimeResources = std::getenv("XTINCT_HYPHENATION_RESOURCES_DIR");
  const char* resources = runtimeResources && runtimeResources[0] != '\0'
                              ? runtimeResources
                              : HYPHENATION_RESOURCES_DIR;
  std::string path = std::string(resources) + "/" + resourceFile;
