...
class HalDisplay {
 public:
  ...
  // Highest-quality validated full-screen grayscale base for a static sleep
  // image. X3 runs one supported post-condition pass around its clean base;
  // the driver then supplies its mandatory fast settle and OEM gray pre-pass.
  // This is intentionally not an arbitrary repeated-FULL loop.
  void displaySleepGrayscaleBase(bool turnOffScreen = false);
