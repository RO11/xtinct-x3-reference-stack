...
void HalDisplay::displaySleepGrayscaleBase(bool turnOffScreen) {
  // A custom sleep image is static for hours, so use the strongest calibrated
  // grayscale path. On UC8253 this request produces exactly one NORMAL
  // post-condition after the FULL clean base; the driver also performs its
  // required FAST no-op settle. The OEM pre-BW-mid and final gray waveforms
  // follow inside displayGrayscaleBase()/displayGrayBuffer(). UC8279 ignores
  // the pass count and selects its own GC + pre-BW-mid sequence because its LUT
  // format differs and has no hardware-validated repeated-condition mode.
  if (gpio.deviceIsX3()) {
    einkDisplay.requestResync(SleepScreenRefreshPolicy::X3_VALIDATED_POST_CONDITION_PASSES);
  }
  einkDisplay.displayGrayscaleBase(convertRefreshMode(RefreshMode::HALF_REFRESH), turnOffScreen);
}
