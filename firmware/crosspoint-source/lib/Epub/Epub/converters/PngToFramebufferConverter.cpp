...
// The pinned PNG decoder is exactly 47,136 bytes (46.0 KiB) on the X3 ABI due
// to its internal zlib state and bounded scanline buffers.
...
static_assert(PNG_DECODER_SIZE == 47136,
              "PNGdec object size changed; remeasure the X3 decoder heap budget");
constexpr size_t MIN_FREE_HEAP_FOR_PNG = PNG_DECODER_SIZE + 16 * 1024;
static_assert(PNG_MAX_BUFFERED_PIXELS <= 4096,
              "PNGdec scanline storage must remain bounded for the ESP32-C3 heap");
...
bool PngToFramebufferConverter::getDimensionsStatic(const std::string& imagePath, ImageDimensions& out) {
  ...
  if (freeHeap < MIN_FREE_HEAP_FOR_PNG || maxBlock < PNG_DECODER_SIZE) {
    LOG_ERR("PNG", "Not enough heap for PNG decoder (%u free/%u max, need %u/%u)", freeHeap, maxBlock,
            MIN_FREE_HEAP_FOR_PNG, PNG_DECODER_SIZE);
    return false;
  }
...
bool PngToFramebufferConverter::decodeToFramebuffer(const std::string& imagePath, GfxRenderer& renderer,
  ...
  // Heap-allocate the 47,136-byte decoder; it is freed at function exit.
  std::unique_ptr<PNG> png(new (std::nothrow) PNG());
  if (!png) {
