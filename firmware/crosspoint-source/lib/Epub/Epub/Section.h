...
class Section {
 public:
  using LoadFailure = epub::PageLoadFailure;
 ...
 private:
  ...
  LoadFailure lastPageLoadFailure_ = LoadFailure::None;
  // False only while loadPageDuringBuild has moved the shared read/write
  // handle away from its append position. The unwind guard sets it true only
  // after a successful seek-back; loadPage abandons the temp transaction if a
  // restore ever fails.
  bool activeBuildCursorSafe_ = true;
  bool finalizeBuild();
  ...
  std::unique_ptr<Page> loadPageAt(int page, LoadFailure& failure) const;
  ...
  std::unique_ptr<Page> loadPageDuringBuild(int page, LoadFailure& failure);
  bool loadSectionFileImpl(const ReaderRenderSpec& spec);
  bool startBuildImpl(const ReaderRenderSpec& spec, const std::function<void()>& popupFn);
  bool buildSomeMoreImpl(int maxPages);
  std::unique_ptr<Page> loadPageImpl(int page, LoadFailure& failure);
 ...
 public:
  ...
  void abandonBuild();
  void abandonBuildNoThrow() noexcept;
  ...
  LoadFailure lastLoadFailure() const { return lastPageLoadFailure_; }
