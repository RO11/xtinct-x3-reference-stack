...
bool ContainerParser::setup() {
  if (remainingSize == 0 || remainingSize > epub::limits::MAX_CONTAINER_XML_BYTES) {
    failed = true;
    LOG_ERR("CTR", "Rejected invalid container.xml size");
    return false;
  }
...
void XMLCALL ContainerParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (!epub::limits::catchAllocationFailure([&]() { startElementImpl(userData, name, atts); })) {
    self->failed = true;
    LOG_ERR("CTR", "Rejected container.xml: allocation failed in callback");
    if (self->parser) XML_StopParser(self->parser, XML_FALSE);
  }
}

void ContainerParser::startElementImpl(void* userData, const XML_Char* name, const XML_Char** atts) {
  auto* self = static_cast<ContainerParser*>(userData);
  ...
  if (self->state == IN_ROOTFILES && strcmp(name, "rootfile") == 0) {
    ...
    if (mediaType && path && strcmp(mediaType, "application/oebps-package+xml") == 0) {
      if (strnlen(path, epub::limits::MAX_HREF_BYTES + 1) > epub::limits::MAX_HREF_BYTES) {
        self->failed = true;
        if (self->parser) XML_StopParser(self->parser, XML_FALSE);
        return;
      }
...
void ContainerParser::endElementImpl(void* userData, const XML_Char* name) {
  auto* self = static_cast<ContainerParser*>(userData);
  if (self->failed) return;
