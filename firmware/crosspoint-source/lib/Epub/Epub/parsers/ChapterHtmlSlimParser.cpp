...
bool trimAndNormalize(const std::string& str, std::string& result) {
  std::string staged;
  if (str.empty()) {
    result.clear();
    return true;
  ...
  const size_t maximumOutput = end - start + 1;
  if (maximumOutput > epub::limits::MAX_RUBY_TEXT_BYTES ||
      !epub::limits::allocationPreflight(maximumOutput + 1, maximumOutput + 1)) {
    return false;
  }
  if (!epub::limits::catchAllocationFailure([&]() {
        staged.reserve(maximumOutput);
        bool inSpace = false;
...
void ChapterHtmlSlimParser::failParsing(const char* const reason) {
  if (parseFailed_) return;
  parseFailed_ = true;
  LOG_ERR("EHP", "Rejected spine XHTML: %s", reason ? reason : "invalid input");
  if (xmlParser_) XML_StopParser(xmlParser_, XML_FALSE);
}

bool ChapterHtmlSlimParser::pushInlineStyle(const StyleStackEntry& entry) {
  const size_t wanted = inlineStyleStack.size() + 1;
  if (wanted > epub::limits::MAX_STYLE_STACK_ENTRIES) return false;
  const size_t nextCapacity = wanted <= inlineStyleStack.capacity()
                                  ? inlineStyleStack.capacity()
                                  : std::min(epub::limits::MAX_STYLE_STACK_ENTRIES,
                                             std::max(wanted, inlineStyleStack.capacity() * 2));
  const size_t retained = nextCapacity * sizeof(StyleStackEntry) * 2 +
                          blockStyleStack.capacity() * sizeof(BlockStyle) * 2;
  if (retained > epub::limits::MAX_STYLE_STACK_BYTES ||
      !epub::limits::checkedVectorPushBack(inlineStyleStack, entry,
                                            epub::limits::MAX_STYLE_STACK_ENTRIES)) return false;
  styleStackBytes_ = retained;
  return true;
}

bool ChapterHtmlSlimParser::pushBlockStyle(const BlockStyle& style) {
  const size_t wanted = blockStyleStack.size() + 1;
  if (wanted > epub::limits::MAX_STYLE_STACK_ENTRIES) return false;
  const size_t nextCapacity = wanted <= blockStyleStack.capacity()
                                  ? blockStyleStack.capacity()
                                  : std::min(epub::limits::MAX_STYLE_STACK_ENTRIES,
                                             std::max(wanted, blockStyleStack.capacity() * 2));
  const size_t retained = inlineStyleStack.capacity() * sizeof(StyleStackEntry) * 2 +
                          nextCapacity * sizeof(BlockStyle) * 2;
  if (retained > epub::limits::MAX_STYLE_STACK_BYTES ||
      !epub::limits::checkedVectorPushBack(blockStyleStack, style,
                                            epub::limits::MAX_STYLE_STACK_ENTRIES)) return false;
  styleStackBytes_ = retained;
  return true;
}
...
void ChapterHtmlSlimParser::pushDecorationStyleEntry(const CssTextDecoration defaultDecoration,
  ...
  if (!pushInlineStyle(entry)) {
    failParsing("inline style stack allocation refused");
    return;
  }
...
void ChapterHtmlSlimParser::flushPendingAnchor() {
  ...
  if (pendingAnchorId.size() > epub::limits::MAX_ANCHOR_BYTES ||
      anchorData.size() >= epub::limits::MAX_ANCHORS_PER_SPINE) {
    failParsing("anchor map exceeds limit");
  ...
  if (pendingAnchorIsToc) {
    if (currentPage && !currentPage->elements.empty()) {
      if (completedPageCount >= static_cast<int>(epub::limits::MAX_PAGES_PER_SPINE)) {
        failParsing("page count exceeds per-spine limit");
        return;
      }
      if (!completePageFn(std::move(currentPage), xpathParagraphIndex, xpathListItemIndex,
                          currentPageVisibleOffset)) {
        failParsing("failed to persist TOC boundary page");
        return;
      }
      completedPageCount++;
      currentPage.reset(new (std::nothrow) Page());
      if (!currentPage) {
        failParsing("out of memory creating TOC boundary page");
        return;
      }
      currentPageNextY = 0;
  ...
  std::pair<std::string, uint16_t> anchor{std::move(pendingAnchorId), static_cast<uint16_t>(completedPageCount)};
  if (!epub::limits::checkedVectorPushBack(anchorData, std::move(anchor),
                                            epub::limits::MAX_ANCHORS_PER_SPINE)) {
    failParsing("anchor map allocation refused");
    return;
  }
  pendingAnchorId.clear();
  pendingAnchorIsToc = false;
}
...
void ChapterHtmlSlimParser::flushPartWordBuffer() {
  ...
  if (!currentTextBlock ||
    ...
    failParsing("paragraph token limit or allocation failure");
    return;
  }
...
void ChapterHtmlSlimParser::startNewTextBlock(const BlockStyle& blockStyle) {
  ...
  currentTextBlock = makeUniqueNoThrow<ParsedText>(extraParagraphSpacing, hyphenationEnabled, focusReadingEnabled,
                                                   blockStyle);
  if (!currentTextBlock) {
    failParsing("out of memory creating paragraph");
    return;
  }
...
void ChapterHtmlSlimParser::emitHorizontalRule(const BlockStyle& blockStyle) {
  ...
  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      failParsing("out of memory creating horizontal-rule page");
      return;
    }
    currentPageNextY = 0;
  }
  ...
  if (!currentPage->elements.empty() && currentPageNextY + totalHeight > viewportHeight) {
    ...
    if (!completePageFn(std::move(currentPage), xpathParagraphIndex, xpathListItemIndex,
                        currentPageVisibleOffset)) {
      failParsing("failed to persist horizontal-rule page");
      return;
    }
    completedPageCount++;
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      failParsing("out of memory creating page after horizontal rule");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
  ...
  if (!pageRule) {
    failParsing("out of memory creating horizontal rule");
    return;
  }
  if (!currentPage->tryAddElement(std::move(pageRule))) {
    failParsing("horizontal-rule page budget exceeded");
    return;
  }
  setCurrentPageVisibleOffset(visibleTextOffset);
...
void ChapterHtmlSlimParser::startElementImpl(void* userData, const XML_Char* name, const XML_Char** atts) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  if (self->parseFailed_) return;
  if (self->depth >= static_cast<int>(epub::limits::MAX_HTML_ELEMENT_DEPTH)) {
    self->failParsing("HTML nesting depth exceeds limit");
    return;
  }
  ...
  if (atts != nullptr) {
    for (int i = 0; atts[i]; i += 2) {
      if (strcmp(atts[i], "class") == 0) {
        if (strnlen(atts[i + 1], epub::limits::MAX_HTML_ATTRIBUTE_BYTES + 1) >
            epub::limits::MAX_HTML_ATTRIBUTE_BYTES) {
          self->failParsing("class attribute exceeds limit");
          return;
        }
        if (!epub::limits::checkedStringAssign(classAttr, atts[i + 1],
                                                epub::limits::MAX_HTML_ATTRIBUTE_BYTES)) {
          self->failParsing("class attribute allocation refused");
          return;
        }
      } else if (strcmp(atts[i], "style") == 0) {
        if (strnlen(atts[i + 1], epub::limits::MAX_HTML_ATTRIBUTE_BYTES + 1) >
            epub::limits::MAX_HTML_ATTRIBUTE_BYTES) {
          self->failParsing("style attribute exceeds limit");
          return;
        }
        if (!epub::limits::checkedStringAssign(styleAttr, atts[i + 1],
                                                epub::limits::MAX_HTML_ATTRIBUTE_BYTES)) {
          self->failParsing("style attribute allocation refused");
          return;
        }
        ...
        if (strnlen(idValue, epub::limits::MAX_ANCHOR_BYTES + 1) > epub::limits::MAX_ANCHOR_BYTES) {
          self->failParsing("anchor id exceeds limit");
          return;
        }
        auto tocIt = std::find(self->tocAnchors.begin(), self->tocAnchors.end(), idValue);
        const bool isTocAnchor = tocIt != self->tocAnchors.end();
        if (isTocAnchor || (!isNonNavigableInlineElement(name) &&
                            self->anchorData.size() < epub::limits::MAX_ANCHORS_PER_SPINE)) {
          ...
          if (isTocAnchor) {
            // Transfer the TOC string allocation into the emitted anchor instead of
            // retaining a second copy for the parser's anchor map.
            self->pendingAnchorId = std::move(*tocIt);
            self->pendingAnchorIsToc = true;
          } else {
            const size_t retained = epub::limits::anchorRetainedBytes(strlen(idValue));
            if (!self->anchorBudget.tryRetain(retained) ||
                !epub::limits::checkedStringAssign(self->pendingAnchorId, idValue,
                                                    epub::limits::MAX_ANCHOR_BYTES)) {
              self->failParsing("anchor aggregate allocation refused");
              return;
            }
            self->pendingAnchorIsToc = false;
          }
        }
      } else if (strcmp(atts[i], "dir") == 0) {
        if (strnlen(atts[i + 1], 16) >= 16) {
          self->failParsing("direction attribute exceeds limit");
          return;
        }
        if (!epub::limits::checkedStringAssign(dirAttr, atts[i + 1], 15)) {
          self->failParsing("direction attribute allocation refused");
          return;
        }
      }
    }
  ...
  if (self->tableDepth == 1 && (strcmp(name, "td") == 0 || strcmp(name, "th") == 0)) {
    ...
    if (!self->pushInlineStyle(headerStyle)) {
      self->failParsing("table header style stack allocation refused");
      return;
    }
  ...
  if (matches(name, IMAGE_TAGS, std::size(IMAGE_TAGS))) {
    ...
    if (atts != nullptr) {
      for (int i = 0; atts[i]; i += 2) {
        if (strcmp(atts[i], "src") == 0) {
          if (strnlen(atts[i + 1], epub::limits::MAX_HREF_BYTES + 1) > epub::limits::MAX_HREF_BYTES) {
            self->failParsing("image source exceeds limit");
            return;
          }
          if (!epub::limits::checkedStringAssign(src, atts[i + 1], epub::limits::MAX_HREF_BYTES)) {
            self->failParsing("image source allocation refused");
            return;
          }
        } else if (src.empty() && (strcmp(atts[i], "href") == 0 || strcmp(atts[i], "xlink:href") == 0)) {
          if (strnlen(atts[i + 1], epub::limits::MAX_HREF_BYTES + 1) > epub::limits::MAX_HREF_BYTES) {
            self->failParsing("image href exceeds limit");
            return;
          }
          if (!epub::limits::checkedStringAssign(src, atts[i + 1], epub::limits::MAX_HREF_BYTES)) {
            self->failParsing("image href allocation refused");
            return;
          }
        } else if (strcmp(atts[i], "alt") == 0) {
          if (strnlen(atts[i + 1], epub::limits::MAX_TITLE_BYTES + 1) > epub::limits::MAX_TITLE_BYTES) {
            self->failParsing("image alt text exceeds limit");
            return;
          }
          if (!epub::limits::checkedStringAssign(alt, atts[i + 1], epub::limits::MAX_TITLE_BYTES)) {
            self->failParsing("image alt allocation refused");
            return;
          }
        }
      }
      ...
      if (!src.empty() && self->imageRendering != 1) {
        ...
        {
          // Resolve the image path relative to the HTML file. Preflight the
          // largest input allocation before URI decoding/normalisation.
          if (!epub::limits::countCanGrow(self->contentBase.size(), src.size(),
                                          epub::limits::MAX_PAGE_IMAGE_PATH_BYTES) ||
              !epub::limits::allocationPreflight(self->contentBase.size() + src.size() + 1,
                                                  self->contentBase.size() + src.size() + 1)) {
            self->failParsing("resolved image path allocation refused");
            return;
          }
              ...
              if (gotDimensions) {
                ...
                if (self->currentPage && !self->currentPage->elements.empty() &&
                    (self->currentPageNextY + imageMarginTop + displayHeight + imageMarginBottom >
                     self->viewportHeight)) {
                  if (self->completedPageCount >= static_cast<int>(epub::limits::MAX_PAGES_PER_SPINE)) {
                    self->failParsing("page count exceeds per-spine limit");
                    return;
                  }
                  if (!self->completePageFn(std::move(self->currentPage), self->xpathParagraphIndex,
                                            self->xpathListItemIndex, self->currentPageVisibleOffset)) {
                    self->failParsing("failed to persist image page");
                    return;
                  }
                  self->completedPageCount++;
                  self->currentPage.reset(new (std::nothrow) Page());
                  if (!self->currentPage) {
                    self->failParsing("out of memory creating image page");
                    return;
                  }
                  ...
                  self->currentPage.reset(new (std::nothrow) Page());
                  if (!self->currentPage) {
                    self->failParsing("out of memory creating initial image page");
                    return;
                  }
                  self->currentPageNextY = 0;
                  self->currentPageVisibleOffsetSet = false;
                ...
                // Transfer both path allocations into the page. Keeping shared
                // copies here doubled path payload at peak on image-heavy pages.
                auto imageBlock = std::unique_ptr<ImageBlock>(new (std::nothrow)
                    ImageBlock(std::move(cachedImagePath), std::move(resolvedPath), displayWidth, displayHeight));
                if (!imageBlock) {
                ...
                if (!pageImage) {
                  self->failParsing("out of memory creating page image");
                  return;
                }
                if (!self->currentPage->tryAddElement(std::move(pageImage))) {
                  self->failParsing("image page budget exceeded");
                  return;
                }
  ...
  if (strcmp(name, "ruby") == 0) {
    ...
    if (self->currentTextBlock && !self->currentTextBlock->ensureRubyCapacity()) {
      self->failParsing("ruby slot memory budget exceeded");
      return;
    }
  ...
  if (strcmp(name, "a") == 0) {
    ...
    if (isInternalLink) {
      ...
      applyDirectionToEntry(entry, cssStyle);
      if (!self->pushInlineStyle(entry)) {
        self->failParsing("link style stack allocation refused");
        return;
      }
      self->updateEffectiveInlineStyle();
  ...
  if (matches(name, HEADER_TAGS, std::size(HEADER_TAGS))) {
    ...
    if (!self->pushBlockStyle(accumulated)) {
      self->failParsing("header style stack allocation refused");
      return;
    }
    ...
    if (strcmp(name, "br") == 0) {
      ...
      if (!self->pushBlockStyle(accumulated)) {
        self->failParsing("block style stack allocation refused");
        return;
      }
      self->startNewTextBlock(accumulated.withoutBottom());
      self->updateEffectiveInlineStyle();
      ...
      if (strcmp(name, "li") == 0) {
        if (!self->currentTextBlock->addWord("\xe2\x80\xa2", EpdFontFamily::REGULAR, false, false,
                                             self->visibleTextOffset)) {
          self->failParsing("paragraph token limit while adding list bullet");
          return;
        }
    ...
    applyDirectionToEntry(entry, cssStyle);
    if (!self->pushInlineStyle(entry)) {
      self->failParsing("bold style stack allocation refused");
      return;
    }
    self->updateEffectiveInlineStyle();
    ...
    applyDirectionToEntry(entry, cssStyle);
    if (!self->pushInlineStyle(entry)) {
      self->failParsing("italic style stack allocation refused");
      return;
    }
    self->updateEffectiveInlineStyle();
    ...
    if (!self->pushInlineStyle(entry)) {
      self->failParsing("vertical style stack allocation refused");
      return;
    }
    self->updateEffectiveInlineStyle();
    ...
    if (cssStyle.hasFontWeight() || cssStyle.hasFontStyle() || cssStyle.hasTextDecoration() ||
      ...
      if (!self->pushInlineStyle(entry)) {
        self->failParsing("inline style stack allocation refused");
        return;
      }
      self->updateEffectiveInlineStyle();
    }
...
void ChapterHtmlSlimParser::characterDataImpl(void* userData, const XML_Char* s, const int len) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  if (self->parseFailed_) return;
  if (len < 0 || !epub::limits::countCanGrow(self->expandedTextBytes_, static_cast<size_t>(len),
                                              epub::limits::MAX_EXPANDED_TEXT_BYTES_PER_SPINE)) {
    self->failParsing("expanded text exceeds per-spine limit");
    return;
  }
  self->expandedTextBytes_ += static_cast<size_t>(len);
  ...
  if (self->collectingRubyText) {
    if (static_cast<size_t>(len) > epub::limits::MAX_RUBY_TEXT_BYTES ||
        self->rubyTextBuffer.size() > epub::limits::MAX_RUBY_TEXT_BYTES - static_cast<size_t>(len)) {
      self->failParsing("ruby annotation exceeds limit");
      return;
    }
    if (!epub::limits::checkedStringAppend(
            self->rubyTextBuffer, std::string_view(s, static_cast<size_t>(len)),
            epub::limits::MAX_RUBY_TEXT_BYTES)) {
      self->failParsing("ruby scratch allocation refused");
      return;
    }
    return;
  }
  ...
  if (blockWordCount > softFlushThreshold) {
    ...
    const uint16_t effectiveWidth = (horizontalInset < self->viewportWidth)
                                        ...
                                        : self->viewportWidth;
    self->currentTextBlock->layoutAndExtractLinesOwned(
        self->renderer, self->fontId, effectiveWidth,
        [self](std::unique_ptr<TextBlock> textBlock, const uint32_t offset) {
          self->addLineToPage(std::move(textBlock), offset);
        },
        false);
    if (!self->currentTextBlock->valid() || self->parseFailed_) {
      self->failParsing("soft-flush layout allocation refused");
      return;
    }
  }
}
...
void XMLCALL ChapterHtmlSlimParser::defaultHandlerExpand(void* userData, const XML_Char* s, const int len) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  if (!epub::limits::catchAllocationFailure([&]() { defaultHandlerExpandImpl(userData, s, len); })) {
    self->failParsing("allocation failed in entity callback");
  }
}

void ChapterHtmlSlimParser::defaultHandlerExpandImpl(void* userData, const XML_Char* s, const int len) {
...
void ChapterHtmlSlimParser::endElementImpl(void* userData, const XML_Char* name) {
  auto* self = static_cast<ChapterHtmlSlimParser*>(userData);
  if (self->parseFailed_) return;
  ...
  if (strcmp(name, "rt") == 0) {
    ...
    if (self->inRuby && self->currentTextBlock) {
      ...
      std::string cleanRuby;
      if (!trimAndNormalize(self->rubyTextBuffer, cleanRuby)) {
        self->failParsing("ruby normalization allocation refused");
        return;
      }
      if (!cleanRuby.empty()) {
        if (baseWordCount > 0) {
          if (!self->currentTextBlock->setRubyGroupAt(self->rubyStartWordIndex, baseWordCount, cleanRuby)) {
            self->failParsing("ruby paragraph budget exceeded");
            return;
          }
          ...
          if (leaderIdx >= 0) {
            const std::string_view prevRuby = self->currentTextBlock->getRubyTextViewAt(leaderIdx);
            if (prevRuby.size() > epub::limits::MAX_RUBY_TEXT_BYTES - cleanRuby.size()) {
              self->failParsing("ruby annotation budget exceeded");
              return;
            }
            const size_t combinedBytes = prevRuby.size() + cleanRuby.size();
            if (!epub::limits::allocationPreflight(combinedBytes + 1, combinedBytes + 1)) {
              self->failParsing("ruby annotation allocation refused");
              return;
            }
            std::string combined(prevRuby);
            combined.reserve(combinedBytes);
            combined.append(cleanRuby);
            if (!self->currentTextBlock->setRubyForWordAt(leaderIdx, combined)) {
              self->failParsing("ruby annotation budget exceeded");
              return;
            }
          }
  ...
  if (self->insideFootnoteLink && self->depth == self->footnoteLinkDepth) {
    if (self->currentFootnote.number[0] != '\0' && self->currentFootnote.href[0] != '\0') {
      ...
      if (self->pendingFootnotes.size() >= epub::limits::MAX_PENDING_FOOTNOTES) {
        self->failParsing("pending footnote count exceeds limit");
        return;
      }
      const size_t wanted = self->pendingFootnotes.size() + 1;
      const size_t nextCapacity = wanted <= self->pendingFootnotes.capacity()
                                      ? self->pendingFootnotes.capacity()
                                      : std::min(epub::limits::MAX_PENDING_FOOTNOTES,
                                                 std::max(wanted, self->pendingFootnotes.capacity() * 2));
      const size_t retained = nextCapacity * sizeof(decltype(self->pendingFootnotes)::value_type);
      if (retained > epub::limits::MAX_PENDING_FOOTNOTE_BYTES ||
          !epub::limits::checkedVectorPushBack(self->pendingFootnotes,
                                                std::make_pair(wordIndex, entry),
                                                epub::limits::MAX_PENDING_FOOTNOTES)) {
        self->failParsing("pending footnote allocation refused");
        return;
      }
      self->pendingFootnoteBytes_ = retained;
    }
...
bool ChapterHtmlSlimParser::beginParse() {
  parseFailed_ = false;
  expandedTextBytes_ = 0;
  size_t requiredAnchorBytes = 0;
  for (const auto& anchor : tocAnchors) {
    const size_t charge = epub::limits::anchorRetainedBytes(anchor.size());
    if (anchor.size() > epub::limits::MAX_ANCHOR_BYTES || charge > SIZE_MAX - requiredAnchorBytes) {
      requiredAnchorBytes = SIZE_MAX;
      break;
    }
    requiredAnchorBytes += charge;
  }
  if (tocAnchors.size() > epub::limits::MAX_ANCHORS_PER_SPINE ||
      requiredAnchorBytes > epub::limits::MAX_RETAINED_ANCHOR_BYTES ||
      (anchorBudget.used() < requiredAnchorBytes &&
       !anchorBudget.tryRetain(requiredAnchorBytes - anchorBudget.used())) ||
      !epub::limits::checkedVectorReserve(anchorData, tocAnchors.size(),
                                           epub::limits::MAX_ANCHORS_PER_SPINE)) {
    failParsing("TOC anchor list exceeds limit");
    return false;
  }
  ...
  styleStackBytes_ = 0;
  if (!pushBlockStyle(rootBlockStyle)) {
    failParsing("root style stack allocation refused");
    return false;
  }
  ...
  if (!epub::limits::inflatedSpineFits(parseFile_.size())) {
    LOG_ERR("EHP", "Spine HTML exceeds safety limit");
    abortParse();
    return false;
  }
...
ChapterHtmlSlimParser::ParseStatus ChapterHtmlSlimParser::parseStep() {
  if (parseFailed_ || !xmlParser_) return ParseStatus::Error;
  ...
  if (parseFailed_) return ParseStatus::Error;
...
bool ChapterHtmlSlimParser::finishParse() {
  ...
  if (currentTextBlock) {
    ...
    if (!completePageFn(std::move(currentPage), xpathParagraphIndex, xpathListItemIndex,
                        currentPageVisibleOffset)) {
      failParsing("failed to persist trailing page");
      abortParse();
      return false;
    }
    completedPageCount++;
...
void ChapterHtmlSlimParser::addLineToPage(std::unique_ptr<TextBlock> line, const uint32_t visibleOffset) {
  if (parseFailed_) return;
  if (!line) {
    failParsing("missing extracted text line");
    return;
  }
  ...
  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      failParsing("out of memory creating text page");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
  ...
  if (currentPageNextY + lineHeight > viewportHeight) {
    ...
    if (!completePageFn(std::move(currentPage), xpathParagraphIndex, xpathListItemIndex,
                        currentPageVisibleOffset)) {
      failParsing("failed to persist text page");
      return;
    }
    completedPageCount++;
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      failParsing("out of memory creating next text page");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
  ...
  while (footnoteIt != pendingFootnotes.end() && footnoteIt->first <= wordsExtractedInBlock) {
    if (!currentPage->addFootnote(footnoteIt->second.number, footnoteIt->second.href)) {
      failParsing("page footnote budget exceeded");
      return;
    }
    ++footnoteIt;
  }
  ...
  auto pageLine = std::unique_ptr<PageLine>(new (std::nothrow)
      PageLine(std::move(line), xOffset, currentPageNextY));
  if (!pageLine || !currentPage->tryAddElement(std::move(pageLine))) {
    failParsing("text page budget exceeded");
    return;
  }
...
void ChapterHtmlSlimParser::makePages() {
  ...
  if (!currentPage) {
    currentPage.reset(new (std::nothrow) Page());
    if (!currentPage) {
      failParsing("out of memory creating layout page");
      return;
    }
    currentPageNextY = 0;
    currentPageVisibleOffsetSet = false;
  ...
  currentTextBlock->layoutAndExtractLinesOwned(
      renderer, fontId, effectiveWidth,
      [this](std::unique_ptr<TextBlock> textBlock, const uint32_t offset) {
        addLineToPage(std::move(textBlock), offset);
      });
  if (!currentTextBlock->valid() || parseFailed_) {
    failParsing("paragraph layout allocation refused");
    return;
  }
  ...
  if (!pendingFootnotes.empty() && currentPage) {
    for (const auto& [idx, fn] : pendingFootnotes) {
      if (!currentPage->addFootnote(fn.number, fn.href)) {
        failParsing("trailing page footnote budget exceeded");
        return;
      }
    }
