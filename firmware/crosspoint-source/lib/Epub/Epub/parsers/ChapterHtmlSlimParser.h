...
class ChapterHtmlSlimParser {
  ...
  std::function<bool(std::unique_ptr<Page>, uint16_t, uint16_t, uint32_t)> completePageFn;
  ...
  bool pendingAnchorIsToc = false;
  std::vector<std::string> tocAnchors;  // the list of anchors that are TOC chapter boundaries
  epub::limits::AnchorBudget anchorBudget;
  ...
  size_t pendingFootnoteBytes_ = 0;
  size_t styleStackBytes_ = 0;
  ...
  size_t expandedTextBytes_ = 0;
  bool parseFailed_ = false;

  void updateEffectiveInlineStyle();
  void failParsing(const char* reason);
  bool pushInlineStyle(const StyleStackEntry& entry);
  bool pushBlockStyle(const BlockStyle& style);
  ...
  static void startElementImpl(void* userData, const XML_Char* name, const XML_Char** atts);
  static void characterDataImpl(void* userData, const XML_Char* s, int len);
  static void defaultHandlerExpandImpl(void* userData, const XML_Char* s, int len);
  static void endElementImpl(void* userData, const XML_Char* name);
 ...
 public:
  explicit ChapterHtmlSlimParser(
      ...
      const std::function<bool(std::unique_ptr<Page>, uint16_t, uint16_t, uint32_t)>& completePageFn,
      ...
      epub::limits::AnchorBudget anchorBudget = epub::limits::AnchorBudget(epub::limits::MAX_RETAINED_ANCHOR_BYTES),
      ...
      : epub(epub),
        ...
        anchorBudget(std::move(anchorBudget)) {}
