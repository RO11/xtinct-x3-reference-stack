...
static_assert(sizeof(std::string) <= epub::limits::RETAINED_RUBY_SLOT_FIXED_BYTES,
              "retained paragraph accounting must cover a libstdc++ string slot");
...
bool ParsedText::addWord(const std::string_view inputWord, const EpdFontFamily::Style fontStyle, const bool underline,
                         const bool attachToPrevious, const uint32_t visibleTextOffset) {
  if (!valid_) return false;
  if (inputWord.empty()) return true;
  // Check both the per-input cap and the minimum retained cost before creating
  // the std::string used by NFC normalization. This keeps a hostile callback
  // from allocating a doomed input/normalization pair near the paragraph cap.
  const size_t minimumRetained = epub::limits::retainedTokenBytes(inputWord.size(), !rubyTexts.empty());
  if (inputWord.size() > epub::limits::MAX_INPUT_WORD_BYTES ||
      minimumRetained > retainedBudget_.remaining() ||
      !epub::limits::allocationPreflight(inputWord.size() * 2 + 2, inputWord.size() + 1)) {
    valid_ = false;
    return false;
  }

  // Arduino defines a function-like `word(...)` macro, so keep this local name
  // macro-safe: this production unit is compiled with Arduino.h transitively.
  ...
  normalizedWord = utf8ComposeNfc(normalizedWord);
  if (normalizedWord.size() > epub::limits::MAX_INPUT_WORD_BYTES) {
    valid_ = false;
    return false;
  }
  ...
  const bool wordStartsRtl = !hasRtlWord && mayContainRtlBytes(normalizedWord.c_str()) &&
                             BidiUtils::startsWithRtl(normalizedWord.c_str(), RTL_PER_WORD_PROBE_DEPTH);

  const auto pushToken = [&](const std::string_view token, const EpdFontFamily::Style tokenStyle,
                             const bool continues, const bool noSpaceBefore, const bool isFocusSuffix,
                             const uint32_t tokenOffset) {
    const size_t retained = epub::limits::retainedTokenBytes(token.size(), !rubyTexts.empty());
    const size_t wanted = words.size() + 1;
    if (token.empty() || token.size() > epub::limits::MAX_INPUT_WORD_BYTES ||
        !epub::limits::paragraphTokensFit(words.size(), 1) || retained > retainedBudget_.remaining() ||
        !epub::limits::allocationPreflight(retained, std::max<size_t>(token.size() + 1, 2048)) ||
        !epub::limits::checkedVectorReserve(wordStyles, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
        !epub::limits::checkedVectorReserve(wordContinues, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
        !epub::limits::checkedVectorReserve(wordNoSpaceBefore, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
        !epub::limits::checkedVectorReserve(wordIsFocusSuffix, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
        !epub::limits::checkedVectorReserve(wordVisibleOffsetDeltas, wanted,
                                             epub::limits::MAX_PARAGRAPH_TOKENS)) {
      valid_ = false;
      return false;
    }
    uint32_t base = visibleOffsetBase;
    if (!wordVisibleOffsetDeltas.empty() && !visibleOffsetRebases.empty()) base = visibleOffsetRebases.back().base;
    const bool needsRebase = !wordVisibleOffsetDeltas.empty() &&
                             (tokenOffset < base || tokenOffset - base > std::numeric_limits<uint16_t>::max());
    if (needsRebase && !epub::limits::checkedVectorReserve(
                           visibleOffsetRebases, visibleOffsetRebases.size() + 1,
                           epub::limits::MAX_PARAGRAPH_TOKENS)) {
      valid_ = false;
      return false;
    }
    std::string staged;
    if (!epub::limits::checkedStringAssign(staged, token, epub::limits::MAX_INPUT_WORD_BYTES) ||
        !retainedBudget_.tryRetain(retained)) {
      valid_ = false;
      return false;
    }
    if (!epub::limits::checkedDequePushBack(words, std::move(staged),
                                            epub::limits::MAX_PARAGRAPH_TOKENS)) {
      retainedBudget_.release(retained);
      valid_ = false;
      return false;
    }
    if (!rubyTexts.empty() &&
        !epub::limits::checkedDequePushBack(rubyTexts, std::string{},
                                            epub::limits::MAX_PARAGRAPH_TOKENS)) {
      words.pop_back();
      retainedBudget_.release(retained);
      valid_ = false;
  ...
  if (attachToPrevious && !words.empty() &&
      hasCjkBreakOpportunityBetween(lastCodepoint(words.back()), firstCodepoint(normalizedWord))) {
    effectiveAttachToPrevious = false;
    effectiveNoSpaceBefore = true;
  }

  // Validate burst counts without speculative reserve(). Vector capacity may
  // grow to at most roughly 2x live size and is included in the conservative
  // per-token retained charge; reserving a hostile worst-case burst up front
  // would allocate capacity for tokens the byte budget will never admit.
  const auto ensureTokenCapacity = [&](const size_t additionalTokens) {
    if (!epub::limits::paragraphTokensFit(words.size(), additionalTokens)) {
      valid_ = false;
      return false;
    }
    return true;
  ...
  if (auto breakOffsets = cjkCharacterBreakByteOffsets(normalizedWord); !breakOffsets.empty()) {
    ...
    if (!ensureTokenCapacity(breakOffsets.size() + 1)) return false;
    bool firstToken = true;
    size_t tokenStart = 0;
    uint32_t tokenVisibleOffset = visibleTextOffset;
    for (const size_t breakOffset : breakOffsets) {
      if (breakOffset <= tokenStart || breakOffset > normalizedWord.size()) continue;
      const std::string_view token =
          std::string_view(normalizedWord).substr(tokenStart, breakOffset - tokenStart);
      if (!pushToken(token, baseStyle, firstToken ? effectiveAttachToPrevious : false,
    ...
    if (tokenStart < normalizedWord.size()) {
      if (!pushToken(std::string_view(normalizedWord).substr(tokenStart), baseStyle,
  ...
  if (containsCjkBreakableCodepoint(normalizedWord)) {
    if (!pushToken(normalizedWord, baseStyle, effectiveAttachToPrevious, effectiveNoSpaceBefore, false,
                   visibleTextOffset)) {
      return false;
    }
    if (wordStartsRtl) {
  ...
  if (!ensureTokenCapacity(normalizedWord.length())) return false;
  ...
  auto processSegment = [&](std::string_view segment, bool isWord, bool attach, bool noSpaceBefore) {
    const size_t maximumAdditional = isWord ? 2 : 1;
    if (!epub::limits::paragraphTokensFit(words.size(), maximumAdditional)) {
      valid_ = false;
      return false;
    }
    const unsigned char* wordBegin = reinterpret_cast<const unsigned char*>(normalizedWord.data());
    ...
    if (!isWord) {
      ...
      return pushToken(segment, baseStyle, attach, noSpaceBefore, false, segmentOffset);
    } else {
      size_t charCount = 0;
      ...
      if (targetBoldChars >= charCount) {
        ...
        return pushToken(segment, static_cast<EpdFontFamily::Style>(baseStyle | EpdFontFamily::BOLD), attach,
                         noSpaceBefore, false, segmentOffset);
      } else {
        ...
        // Bold prefix
        if (!pushToken(segment.substr(0, splitByteOffset),
                       static_cast<EpdFontFamily::Style>(baseStyle | EpdFontFamily::BOLD), attach, noSpaceBefore,
                       false, segmentOffset)) {
          return false;
        }

        // Regular suffix - marked so extractLine can merge it back into single TextBlock entry
        return pushToken(segment.substr(splitByteOffset), baseStyle, true, false, true,
                         segmentOffset + static_cast<uint32_t>(targetBoldChars));
      }
    }
    return true;
  };
  ...
  const unsigned char* ptr = reinterpret_cast<const unsigned char*>(normalizedWord.c_str());
  const unsigned char* end = ptr + normalizedWord.length();
...
bool ParsedText::setRubyForWordAt(size_t index, const std::string& ruby) {
  if (!valid_ || index >= words.size() || ruby.size() > epub::limits::MAX_RUBY_TEXT_BYTES ||
      !enableRubySlots()) {
    valid_ = false;
    return false;
  }
  const size_t previousBytes = rubyTexts[index].size();
  const size_t retainedBytes = rubyBytes_ >= previousBytes ? rubyBytes_ - previousBytes : 0;
  if (!epub::limits::rubyTextFits(retainedBytes, ruby.size())) {
    valid_ = false;
    return false;
  }

  const size_t previousCharge = epub::limits::retainedRubyPayloadBytes(previousBytes);
  const size_t nextCharge = epub::limits::retainedRubyPayloadBytes(ruby.size());
  std::string stagedRuby;
  if (!epub::limits::checkedStringAssign(stagedRuby, ruby, epub::limits::MAX_RUBY_TEXT_BYTES)) {
    valid_ = false;
    return false;
  }
  if (nextCharge > previousCharge) {
    if (!retainedBudget_.tryRetain(nextCharge - previousCharge)) {
      valid_ = false;
      return false;
    }
  } else {
    retainedBudget_.release(previousCharge - nextCharge);
  }
  rubyTexts[index].swap(stagedRuby);
  rubyBytes_ = retainedBytes + ruby.size();
  return true;
}

bool ParsedText::setRubyGroupAt(size_t startIndex, size_t count, const std::string& ruby) {
  if (!valid_ || startIndex >= words.size() || count == 0 || count > words.size() - startIndex ||
      ruby.size() > epub::limits::MAX_RUBY_TEXT_BYTES || !enableRubySlots()) {
    valid_ = false;
    return false;
  }
  size_t previousBytes = 0;
  size_t previousCharge = 0;
  for (size_t i = 0; i < count; i++) {
    const size_t bytes = rubyTexts[startIndex + i].size();
    previousBytes += bytes;
    previousCharge += epub::limits::retainedRubyPayloadBytes(bytes);
  }
  const size_t retainedBytes = rubyBytes_ >= previousBytes ? rubyBytes_ - previousBytes : 0;
  if (!epub::limits::rubyTextFits(retainedBytes, ruby.size())) {
    valid_ = false;
  ...
  rubyTexts[startIndex].swap(stagedRuby);
  rubyBytes_ = retainedBytes + ruby.size();
  for (size_t i = 1; i < count; i++) {
    size_t idx = startIndex + i;
    std::string{}.swap(rubyTexts[idx]);
    wordStyles[idx] =
...
bool ParsedText::enableRubySlots() {
  if (!rubyTexts.empty() || words.empty()) return true;
  if (words.size() > SIZE_MAX / epub::limits::RETAINED_RUBY_SLOT_FIXED_BYTES) return false;
  const size_t retained = words.size() * epub::limits::RETAINED_RUBY_SLOT_FIXED_BYTES;
  if (!retainedBudget_.tryRetain(retained)) return false;
  std::deque<std::string> staged;
  if (!epub::limits::checkedDequeResize(staged, words.size(),
                                        epub::limits::MAX_PARAGRAPH_TOKENS)) {
    retainedBudget_.release(retained);
    return false;
  }
  rubyTexts.swap(staged);
  return true;
}

bool ParsedText::ensureRubyCapacity() {
  if (!valid_ || !enableRubySlots()) {
    valid_ = false;
    return false;
  }
  return true;
...
void ParsedText::releaseRetainedPrefix(const size_t count) {
  const size_t releasedCount = std::min(count, words.size());
  const bool rubySlotsActive = !rubyTexts.empty();
  size_t released = 0;
  size_t releasedRubyBytes = 0;
  for (size_t i = 0; i < releasedCount; i++) {
    released += epub::limits::retainedTokenBytes(words[i].size(), rubySlotsActive);
    if (rubySlotsActive && i < rubyTexts.size()) {
      releasedRubyBytes += rubyTexts[i].size();
      released += epub::limits::retainedRubyPayloadBytes(rubyTexts[i].size());
    }
  }
  retainedBudget_.release(released);
  rubyBytes_ = releasedRubyBytes >= rubyBytes_ ? 0 : rubyBytes_ - releasedRubyBytes;
}
...
void ParsedText::layoutAndExtractLinesOwned(
    const GfxRenderer& renderer, const int fontId, const uint16_t viewportWidth,
    const std::function<void(std::unique_ptr<TextBlock>, uint32_t)>& processLine, const bool includeLastLine) {
  if (words.empty()) {
    return;
  }

  const size_t scratchPerWord = 112;
  if (words.size() > (epub::limits::MAX_LAYOUT_SCRATCH_BYTES - 4096) / scratchPerWord) {
    valid_ = false;
    LOG_ERR("PTX", "Layout scratch exceeds retained-memory envelope");
    return;
  }
  const size_t layoutScratch = 4096 + words.size() * scratchPerWord;
  const size_t largestScratch = words.size() * sizeof(std::string) * 2;
  if (!epub::limits::allocationPreflight(layoutScratch, largestScratch)) {
    valid_ = false;
    LOG_ERR("PTX", "Layout allocation preflight refused");
    return;
  }
  ...
  if (lineCount > 0) {
    ...
    if (!rubyTexts.empty()) {
      ...
      // Once all remaining annotations are empty, release the dense ruby slot
      // table and its retained charge. A later ruby element can enable it again.
      const bool anyRuby = std::any_of(rubyTexts.begin(), rubyTexts.end(),
                                       [](const std::string& value) { return !value.empty(); });
      if (!anyRuby) {
        retainedBudget_.release(rubyTexts.size() * epub::limits::RETAINED_RUBY_SLOT_FIXED_BYTES);
        std::deque<std::string>{}.swap(rubyTexts);
        rubyBytes_ = 0;
      }
    }
  }
...
void ParsedText::layoutAndExtractLines(
    const GfxRenderer& renderer, const int fontId, const uint16_t viewportWidth,
    const std::function<void(std::shared_ptr<TextBlock>, uint32_t)>& processLine, const bool includeLastLine) {
  layoutAndExtractLinesOwned(
      renderer, fontId, viewportWidth,
      [this, &processLine](std::unique_ptr<TextBlock> block, const uint32_t offset) {
        if (!block || !epub::limits::allocationPreflight(sizeof(TextBlock) + 64, sizeof(TextBlock) + 64)) {
          valid_ = false;
          return;
        }
        // Compatibility path for settings preview. The untrusted EPUB section
        // builder uses layoutAndExtractLinesOwned and never allocates a shared
        // control block.
        std::shared_ptr<TextBlock> shared(block.release());
        processLine(std::move(shared), offset);
      },
      includeLastLine);
}
...
bool ParsedText::hyphenateWordAtIndex(const size_t wordIndex, const int availableWidth, const GfxRenderer& renderer,
  ...
  // A split retains one extra deque/string slot and one extra entry in every
  // parallel array (plus an optional rendered hyphen). Charge that delta before
  // allocating the remainder substring or inserting into any container.
  if (!epub::limits::paragraphTokensFit(words.size(), 1)) return false;
  const bool rubySlotsActive = !rubyTexts.empty();
  const size_t prefixBytes = chosenOffset + (chosenNeedsHyphen ? 1 : 0);
  const size_t remainderBytes = word.size() - chosenOffset;
  const size_t oldRetained = epub::limits::retainedTokenBytes(word.size(), rubySlotsActive);
  const size_t splitRetained = epub::limits::retainedTokenBytes(prefixBytes, rubySlotsActive) +
                               epub::limits::retainedTokenBytes(remainderBytes, rubySlotsActive);
  const size_t retainedDelta = splitRetained >= oldRetained ? splitRetained - oldRetained : SIZE_MAX;
  const size_t wanted = words.size() + 1;
  if (retainedDelta > retainedBudget_.remaining() ||
      !epub::limits::allocationPreflight(retainedDelta + prefixBytes + remainderBytes + 2,
                                          std::max(prefixBytes, remainderBytes) + 1) ||
      !epub::limits::checkedVectorReserve(wordStyles, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
      !epub::limits::checkedVectorReserve(wordContinues, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
      !epub::limits::checkedVectorReserve(wordNoSpaceBefore, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
      !epub::limits::checkedVectorReserve(wordIsFocusSuffix, wanted, epub::limits::MAX_PARAGRAPH_TOKENS) ||
      !epub::limits::checkedVectorReserve(wordVisibleOffsetDeltas, wanted,
                                           epub::limits::MAX_PARAGRAPH_TOKENS) ||
      !epub::limits::checkedVectorReserve(wordWidths, wanted, epub::limits::MAX_PARAGRAPH_TOKENS)) return false;
  ...
  uint32_t visibleBase = visibleOffsetBaseAt(wordIndex);
  if ((remainderOffset < visibleBase ||
       remainderOffset - visibleBase > std::numeric_limits<uint16_t>::max()) &&
      !epub::limits::checkedVectorReserve(visibleOffsetRebases, visibleOffsetRebases.size() + 1,
                                           epub::limits::MAX_PARAGRAPH_TOKENS)) return false;

  std::string prefix;
  std::string remainder;
  if (!epub::limits::checkedStringAssign(prefix, std::string_view(word).substr(0, chosenOffset),
                                          epub::limits::MAX_INPUT_WORD_BYTES) ||
      !epub::limits::checkedStringAssign(remainder, std::string_view(word).substr(chosenOffset),
                                          epub::limits::MAX_INPUT_WORD_BYTES)) return false;
  if (chosenNeedsHyphen) {
    if (prefix.size() >= epub::limits::MAX_INPUT_WORD_BYTES ||
        (prefix.size() == prefix.capacity() &&
         !epub::limits::allocationPreflight(prefix.size() + 2, prefix.size() + 2))) return false;
    prefix.push_back('-');
  }
  if (!retainedBudget_.tryRetain(retainedDelta)) return false;
  if (!epub::limits::checkedDequePushBack(words, std::move(remainder),
                                           epub::limits::MAX_PARAGRAPH_TOKENS)) {
    retainedBudget_.release(retainedDelta);
    return false;
  }
  if (rubySlotsActive &&
      !epub::limits::checkedDequePushBack(rubyTexts, std::string{},
  ...
  std::rotate(words.begin() + wordIndex + 1, words.end() - 1, words.end());
  if (rubySlotsActive) {
    std::rotate(rubyTexts.begin() + wordIndex + 1, rubyTexts.end() - 1, rubyTexts.end());
  }
  words[wordIndex].swap(prefix);
...
void ParsedText::extractLine(const size_t breakIndex, const int pageWidth, const std::vector<uint16_t>& wordWidths,
  ...
  auto block = std::unique_ptr<TextBlock>(new (std::nothrow) TextBlock(
      outWords, outXPos, outStyles, outBoundaries, outSuffixX, blockStyle, std::move(outRubyTexts)));
  if (!block || !block->valid()) {
    LOG_ERR("PTX", "Dropping line: TextBlock arena allocation failed");
    valid_ = false;
    return;
