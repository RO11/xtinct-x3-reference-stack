...
class ImageBlock final : public Block {
 public:
  ImageBlock(std::string imagePath, std::string srcPath, int16_t width, int16_t height);
  ...
  [[nodiscard]] size_t retainedSize() const;
  [[nodiscard]] size_t retainedPathBytes() const { return imagePath.size() + srcPath.size() + 2; }
  static std::unique_ptr<ImageBlock> deserialize(
