...
size_t ImageBlock::serializedSize() const {
  return 2 * sizeof(uint32_t) + imagePath.size() + srcPath.size() + 2 * sizeof(int16_t);
}

size_t ImageBlock::retainedSize() const {
  return sizeof(ImageBlock) + imagePath.size() + srcPath.size() + 2;
}
...
std::unique_ptr<ImageBlock> ImageBlock::deserialize(
  ...
  int16_t w = 0;
  int16_t h = 0;
  if (!readPageString(file, path, epub::limits::MAX_HREF_BYTES, budget, failure) ||
      !readPageString(file, src, epub::limits::MAX_HREF_BYTES, budget, failure) ||
      !readPagePod(file, w, budget) || !readPagePod(file, h, budget) || path.empty() || w <= 0 || h <= 0 ||
      (budget && !budget->tryRetainImagePaths(path.size() + src.size() + 2))) {
    LOG_ERR("IMG", "Deserialization failed: malformed image block");
    return nullptr;
  }
  if (budget && !budget->tryRetain(epub::limits::RETAINED_IMAGE_BLOCK_FIXED_BYTES, sizeof(ImageBlock))) {
    if (budget->resourceRefused()) epub::limits::markPageDecodeResourceRefusal(failure);
    return nullptr;
  }
  auto* image = new (std::nothrow) ImageBlock(std::move(path), std::move(src), w, h);
  if (!image) epub::limits::markPageDecodeResourceRefusal(failure);
  return std::unique_ptr<ImageBlock>(image);
}
