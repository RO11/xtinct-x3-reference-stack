...
class ParsedText {
  ...
  // can fail under a fragmented, BLE-resident heap (the historical fresh-open
  // CJK crash). A deque grows in fixed ~512 B nodes
  ...
  bool valid_ = true;
  size_t rubyBytes_ = 0;
  epub::limits::RetainedParagraphBudget retainedBudget_;
  ...
  bool enableRubySlots();
  void releaseRetainedPrefix(size_t count);
 ...
 public:
  ...
  bool addWord(std::string_view word, EpdFontFamily::Style fontStyle, bool underline = false,
               bool attachToPrevious = false, uint32_t visibleTextOffset = 0);
  bool setRubyForWordAt(size_t index, const std::string& ruby);
  bool setRubyGroupAt(size_t startIndex, size_t count, const std::string& ruby);
  ...
  std::string_view getRubyTextViewAt(size_t index) const {
    return index < rubyTexts.size() ? std::string_view(rubyTexts[index]) : std::string_view();
  }
  bool ensureRubyCapacity();
  ...
  void layoutAndExtractLinesOwned(const GfxRenderer& renderer, int fontId, uint16_t viewportWidth,
                                  const std::function<void(std::unique_ptr<TextBlock>, uint32_t)>& processLine,
                                  bool includeLastLine = true);
};
