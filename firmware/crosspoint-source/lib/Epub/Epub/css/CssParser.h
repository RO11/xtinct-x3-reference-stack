...
class CssParser {
 ...
 private:
  ...
  using RuleMap = std::unordered_map<std::string, CssStyle, SvHash, SvEqual>;
  RuleMap rulesBySelector_;
  size_t retainedRuleBytes_ = 0;
  bool failed_ = false;
  ...
  bool insertRule(RuleMap& target, size_t& retainedBytes, std::string_view selector, const CssStyle& style);
