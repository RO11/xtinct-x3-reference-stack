...
bool writeExact(HalFile& file, const void* data, const size_t bytes) {
  return bytes == 0 || file.write(static_cast<const uint8_t*>(data), bytes) == bytes;
}

bool readExact(HalFile& file, void* data, const size_t bytes) {
  return bytes == 0 || file.read(static_cast<uint8_t*>(data), bytes) == bytes;
}
...
bool CssParser::insertRule(RuleMap& target, size_t& retainedBytes, const std::string_view selector,
                           const CssStyle& style) {
  if (auto it = target.find(selector); it != target.end()) {
    it->second.applyOver(style);
    return true;
  }
  if (target.size() >= epub::limits::MAX_CSS_RULES) return false;
  const size_t charge = epub::limits::cssRuleRetainedBytes(selector.size());
  if (!epub::limits::countCanGrow(retainedBytes, charge, epub::limits::MAX_RETAINED_CSS_RULE_BYTES) ||
      !epub::limits::allocationPreflight(charge, selector.size() + 1)) {
    return false;
  }
  std::string key;
  if (!epub::limits::checkedStringAssign(key, selector, MAX_SELECTOR_LENGTH)) return false;
  bool inserted = false;
  if (!epub::limits::catchAllocationFailure(
          [&]() { inserted = target.emplace(std::move(key), style).second; })) {
    return false;
  }
...
bool CssParser::processRuleBlockWithStyle(std::string_view selectorGroup, const CssStyle& style) {
  ...
  if (rulesBySelector_.size() >= epub::limits::MAX_CSS_RULES) {
    LOG_ERR("CSS", "CSS rule count exceeds retained-memory limit");
    return false;
  }
  ...
  forEachDelimitedToken(
      ...
      [&](std::string_view sel) {
        ...
        // Store or merge with existing. Count, selector payload, string/node,
        // and bucket slack are charged before the owning insertion.
        accepted = insertRule(rulesBySelector_, retainedRuleBytes_, sel, style);
      });
  return accepted;
}
...
bool CssParser::loadFromStream(HalFile& source) {
  ...
  while (source.available()) {
    ...
    for (int i = 0; i < bytesRead && !failed_; ++i) {
  ...
  if (!failed_ && maybeSlash) {
    handleChar('/');
  }

  if (failed_) {
    LOG_ERR("CSS", "Rejected stylesheet: retained rule budget or allocation preflight failed");
    clear();
    failed_ = true;
    return false;
  }
...
bool CssParser::saveToCache() const {
  if (cachePath.empty() || failed_ || rulesBySelector_.size() > epub::limits::MAX_CSS_RULES ||
      retainedRuleBytes_ > epub::limits::MAX_RETAINED_CSS_RULE_BYTES) return false;
  ...
  const uint8_t version = CssParser::CSS_CACHE_VERSION;
  const auto ruleCount = static_cast<uint16_t>(rulesBySelector_.size());
  if (!writeExact(file, &version, sizeof(version)) || !writeExact(file, &ruleCount, sizeof(ruleCount))) {
    return failWrite();
  }

  // Write each rule: selector string + CssStyle fields
  for (const auto& pair : rulesBySelector_) {
    if (pair.first.empty() || pair.first.size() > MAX_SELECTOR_LENGTH) return failWrite();
    const auto selectorLen = static_cast<uint16_t>(pair.first.size());
    if (!writeExact(file, &selectorLen, sizeof(selectorLen)) ||
        !writeExact(file, pair.first.data(), selectorLen)) return failWrite();

    const CssStyle& style = pair.second;
    const uint8_t enums[] = {static_cast<uint8_t>(style.textAlign), static_cast<uint8_t>(style.fontStyle),
                             static_cast<uint8_t>(style.fontWeight), static_cast<uint8_t>(style.textDecoration),
                             static_cast<uint8_t>(style.direction)};
    if (!writeExact(file, enums, sizeof(enums))) return failWrite();

    auto writeLength = [&file](const CssLength& len) {
      const uint8_t unit = static_cast<uint8_t>(len.unit);
      return writeExact(file, &len.value, sizeof(len.value)) && writeExact(file, &unit, sizeof(unit));
    };

    if (!writeLength(style.textIndent) || !writeLength(style.marginTop) || !writeLength(style.marginBottom) ||
        ...
        !writeLength(style.imageHeight) || !writeLength(style.imageWidth)) return failWrite();
    const uint8_t tailEnums[] = {static_cast<uint8_t>(style.display), static_cast<uint8_t>(style.verticalAlign)};
    if (!writeExact(file, tailEnums, sizeof(tailEnums))) return failWrite();
    ...
    if (!writeExact(file, &definedBits, sizeof(definedBits))) return failWrite();
  }
...
bool CssParser::loadFromCache() {
  ...
  const auto rejectCache = [&](const char* reason) {
    LOG_ERR("CSS", "Rejected CSS cache: %s", reason);
    file.close();
    Storage.remove((cachePath + rulesCache).c_str());
    return false;
  };
  const uint64_t fileSize = file.fileSize64();
  if (fileSize < sizeof(uint8_t) + sizeof(uint16_t)) return rejectCache("truncated header");

  uint8_t version = 0;
  if (!readExact(file, &version, sizeof(version)) || version != CssParser::CSS_CACHE_VERSION)
    return rejectCache("version mismatch");

  uint16_t ruleCount = 0;
  if (!readExact(file, &ruleCount, sizeof(ruleCount)) || ruleCount > epub::limits::MAX_CSS_RULES)
    return rejectCache("invalid rule count");

  auto hasRemainingBytes = [&file](const size_t neededBytes) -> bool {
    return static_cast<size_t>(file.available()) >= neededBytes;
  };
  constexpr size_t MIN_RULE_BYTES = sizeof(uint16_t) + 1 + CSS_FIXED_STYLE_BYTES;
  if (ruleCount > 0 && (!hasRemainingBytes(static_cast<size_t>(ruleCount) * MIN_RULE_BYTES) ||
                        !epub::limits::allocationPreflight(
                            std::min(epub::limits::MAX_RETAINED_CSS_RULE_BYTES,
                                     static_cast<size_t>(ruleCount) * epub::limits::RETAINED_CSS_RULE_FIXED_BYTES),
                            static_cast<size_t>(ruleCount) * sizeof(void*) * 2))) {
    return rejectCache("rule table allocation preflight failed");
  }

  RuleMap staged;
  size_t stagedBytes = 0;
  if (ruleCount > 0 &&
      !epub::limits::catchAllocationFailure([&]() { staged.reserve(ruleCount); })) {
    return rejectCache("rule table allocation failed");
  }

  for (uint16_t i = 0; i < ruleCount; ++i) {
    uint16_t selectorLen = 0;
    if (!readExact(file, &selectorLen, sizeof(selectorLen)) || selectorLen == 0 ||
        selectorLen > MAX_SELECTOR_LENGTH || !hasRemainingBytes(static_cast<size_t>(selectorLen) + CSS_FIXED_STYLE_BYTES))
      return rejectCache("invalid selector or truncated style");

    std::string selector;
    if (!epub::limits::checkedStringResize(selector, selectorLen, MAX_SELECTOR_LENGTH))
      return rejectCache("selector allocation refused");
    if (!readExact(file, selector.data(), selectorLen)) return rejectCache("truncated selector");

    CssStyle style;
    uint8_t enums[5] = {};
    if (!readExact(file, enums, sizeof(enums)) || enums[0] > static_cast<uint8_t>(CssTextAlign::None) ||
        enums[1] > static_cast<uint8_t>(CssFontStyle::Italic) ||
        enums[2] > static_cast<uint8_t>(CssFontWeight::Bold) ||
        (enums[3] & ~CSS_TEXT_DECORATION_MASK) != 0 ||
        enums[4] > static_cast<uint8_t>(CssTextDirection::Rtl)) return rejectCache("invalid style enum");
    style.textAlign = static_cast<CssTextAlign>(enums[0]);
    style.fontStyle = static_cast<CssFontStyle>(enums[1]);
    style.fontWeight = static_cast<CssFontWeight>(enums[2]);
    style.textDecoration = static_cast<CssTextDecoration>(enums[3]);
    style.direction = static_cast<CssTextDirection>(enums[4]);

    auto readLength = [&file](CssLength& len) -> bool {
      uint8_t unitVal = 0;
      if (!readExact(file, &len.value, sizeof(len.value)) || !readExact(file, &unitVal, sizeof(unitVal)) ||
          unitVal > static_cast<uint8_t>(CssUnit::Percent)) return false;
    ...
    if (!readLength(style.textIndent) || !readLength(style.marginTop) || !readLength(style.marginBottom) ||
      ...
      return rejectCache("invalid length payload");
    }

    uint8_t displayVal = 0;
    if (!readExact(file, &displayVal, sizeof(displayVal)) || displayVal > static_cast<uint8_t>(CssDisplay::None))
      return rejectCache("invalid display value");
    style.display = static_cast<CssDisplay>(displayVal);

    uint8_t verticalAlignVal = 0;
    if (!readExact(file, &verticalAlignVal, sizeof(verticalAlignVal)) ||
        verticalAlignVal > static_cast<uint8_t>(CssVerticalAlign::Sub))
      return rejectCache("invalid vertical alignment");
    style.verticalAlign = static_cast<CssVerticalAlign>(verticalAlignVal);

    uint32_t definedBits = 0;
    if (!readExact(file, &definedBits, sizeof(definedBits)) || (definedBits & ~((1U << 18) - 1U)) != 0)
      return rejectCache("invalid property flags");
    ...
    if (!insertRule(staged, stagedBytes, selector, style)) return rejectCache("retained rule budget exceeded");
  }

  if (file.position() != fileSize) return rejectCache("trailing or inconsistent cache bytes");
  file.close();
  rulesBySelector_.swap(staged);
  retainedRuleBytes_ = stagedBytes;
  failed_ = false;
