...
// Finds the first supported image href in an XHTML cover wrapper without ever
// materializing the wrapper. The largest object is one fixed 2 KiB href buffer;
// returning a short write asks ZipFile to stop inflating immediately on match.
 ...
 public:
  ...
  size_t write(const uint8_t* buffer, const size_t size) override {
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      if (complete_) return i + 1;
    }
    return size;
  }

  [[nodiscard]] bool complete() const { return complete_; }
  [[nodiscard]] std::string_view reference() const { return {reference_.data(), referenceLength_}; }

 private:
  static constexpr std::array<std::string_view, 2> PATTERNS = {"xlink:href=\"", "src=\""};
  std::array<size_t, PATTERNS.size()> matched_{};
  std::array<char, epub::limits::MAX_HREF_BYTES + 1> reference_{};
  size_t referenceLength_ = 0;
  bool capturing_ = false;
  bool oversized_ = false;
  bool complete_ = false;

  void resetMatchers() { matched_.fill(0); }

  void consume(const char c) {
    if (capturing_) {
      if (c == '"') {
        if (!oversized_ && referenceLength_ > 0) {
          const std::string_view candidate(reference_.data(), referenceLength_);
          if (FsHelpers::hasPngExtension(candidate) || FsHelpers::hasJpgExtension(candidate)) {
            complete_ = true;
            return;
          }
        }
      ...
      if (!oversized_) {
        if (referenceLength_ >= epub::limits::MAX_HREF_BYTES) {
          oversized_ = true;
          referenceLength_ = 0;
        } else {
          reference_[referenceLength_++] = c;
        }
      }
      return;
    }
    ...
    for (size_t i = 0; i < PATTERNS.size(); ++i) {
      ...
      size_t& matched = matched_[i];
      if (c == pattern[matched]) {
        ++matched;
      } else {
        matched = c == pattern.front() ? 1 : 0;
      }
...
Epub::Epub(const std::string& sourcePath, const char* const cacheDir) noexcept {
  if (!epub::limits::catchAllocationFailure([&]() {
        filepath = sourcePath;
        cachePath = std::string(cacheDir ? cacheDir : "") + "/epub_" +
                    std::to_string(std::hash<std::string>{}(filepath));
      })) {
    filepath.clear();
    cachePath.clear();
    allocationValid_ = false;
  }
}
...
bool Epub::findContentOpfFile(std::string* contentOpfFile) const {
  ...
  if (!getItemSize(containerPath, &containerSize) || containerSize == 0 ||
      containerSize > epub::limits::MAX_CONTAINER_XML_BYTES) {
  ...
  if (!readItemContentsToStream(containerPath, containerParser, 512) || !containerParser.isValid()) {
...
bool Epub::parseContentOpf(BookMetadataCache::BookMetadata& bookMetadata, const bool writeSpineEntries) {
  ...
  if (!getItemSize(contentOpfFilePath, &contentOpfSize) || contentOpfSize == 0 ||
      contentOpfSize > epub::limits::MAX_EPUB_RESOURCE_BYTES) {
    LOG_ERR("EBP", "Could not get size of content.opf");
    return false;
  }

  ParsedOpfResult parsed;
  {
    ...
    if (!readItemContentsToStream(contentOpfFilePath, opfParser, 1024) || !opfParser.isValid()) {
      LOG_ERR("EBP", "Could not read content.opf");
      return false;
    }
    parsed.title = std::move(opfParser.title);
    parsed.author = std::move(opfParser.author);
    parsed.language = std::move(opfParser.language);
    parsed.tocNcxPath = std::move(opfParser.tocNcxPath);
    parsed.tocNavPath = std::move(opfParser.tocNavPath);
    parsed.coverItemHref = std::move(opfParser.coverItemHref);
    parsed.guideCoverPageHref = std::move(opfParser.guideCoverPageHref);
    parsed.textReferenceHref = std::move(opfParser.textReferenceHref);
  ...
  if (!epub::limits::allocationPreflight(parsed.title.size() + 1, parsed.title.size() + 1)) return false;
  bookMetadata.title = utf8ComposeNfc(parsed.title);
  bookMetadata.author = std::move(parsed.author);
  bookMetadata.language = std::move(parsed.language);
  bookMetadata.coverItemHref = std::move(parsed.coverItemHref);

  // Guide-based cover fallback: if no cover found via metadata/properties,
  // try extracting the image reference from the guide's cover page XHTML
  if (bookMetadata.coverItemHref.empty() && !parsed.guideCoverPageHref.empty()) {
    LOG_DBG("EBP", "No cover from metadata, trying guide cover page: %s", parsed.guideCoverPageHref.c_str());
    size_t coverPageSize = 0;
    CoverImageReferenceScanner scanner;
    if (getItemSize(parsed.guideCoverPageHref, &coverPageSize) && coverPageSize > 0 &&
        coverPageSize <= epub::limits::MAX_COVER_WRAPPER_BYTES &&
        readItemContentsToStream(parsed.guideCoverPageHref, scanner, 512, /*allowEarlyStop=*/true) &&
        scanner.complete()) {
      std::string coverPageBase;
      const auto lastSlash = parsed.guideCoverPageHref.rfind('/');
      if (lastSlash != std::string::npos) {
        coverPageBase = parsed.guideCoverPageHref.substr(0, lastSlash + 1);
      }
      const auto ref = scanner.reference();
      if (!epub::limits::countCanGrow(coverPageBase.size(), ref.size(), epub::limits::MAX_HREF_BYTES) ||
          !epub::limits::allocationPreflight(coverPageBase.size() + ref.size() + 1,
                                              coverPageBase.size() + ref.size() + 1)) return false;
      coverPageBase.append(ref);
      bookMetadata.coverItemHref = FsHelpers::normalisePath(FsHelpers::decodeUriEscapes(coverPageBase));
      if (bookMetadata.coverItemHref.size() > epub::limits::MAX_HREF_BYTES) return false;
      LOG_DBG("EBP", "Found cover image from guide: %s", bookMetadata.coverItemHref.c_str());
    }
  }

  bookMetadata.textReferenceHref = std::move(parsed.textReferenceHref);

  if (!parsed.tocNcxPath.empty()) {
    tocNcxItem = std::move(parsed.tocNcxPath);
  }

  if (!parsed.tocNavPath.empty()) {
    tocNavItem = std::move(parsed.tocNavPath);
  }

  cssFiles = std::move(parsed.cssFiles);
...
bool Epub::parseTocNcxFile() const {
  ...
  if (!getItemSize(tocNcxItem, &ncxSize) || ncxSize == 0 ||
      ncxSize > epub::limits::MAX_EPUB_RESOURCE_BYTES) {
  ...
  if (!readItemContentsToStream(tocNcxItem, ncxParser, 1024) || !ncxParser.isValid()) {
...
bool Epub::parseTocNavFile() const {
  ...
  if (!getItemSize(tocNavItem, &navSize) || navSize == 0 ||
      navSize > epub::limits::MAX_EPUB_RESOURCE_BYTES) {
  ...
  // through a temp file on the SD card (decompress -> write -> reopen -> reread -> delete).
  if (!readItemContentsToStream(tocNavItem, navParser, 1024) || !navParser.isValid()) {
...
void Epub::discoverCssFilesFromZip() {
  ...
  ZipFile zf(filepath);
  size_t cssHrefBytes = 0;
  for (const auto& path : cssFiles) {
    if (!epub::limits::countCanGrow(cssHrefBytes, path.size() + 1, epub::limits::MAX_CSS_HREF_BYTES)) {
      std::vector<std::string>{}.swap(cssFiles);
      return;
    }
    cssHrefBytes += path.size() + 1;
  }
  ...
  if (!zf.enumerateFilePaths([&](std::string_view filePath) {
        ...
        const size_t charge = filePath.size() + 1;
        if (filePath.size() > epub::limits::MAX_HREF_BYTES ||
            !epub::limits::countCanGrow(cssHrefBytes, charge, epub::limits::MAX_CSS_HREF_BYTES) ||
            !epub::limits::allocationPreflight(charge, charge)) {
          LOG_ERR("EBP", "CSS discovery exceeded aggregate retained-memory limit");
          aggregateFailed = true;
          return;
        }
        std::string stagedPath(filePath);
        if (!epub::limits::checkedVectorPushBack(cssFiles, std::move(stagedPath),
                                                 epub::limits::MAX_CSS_FILES)) {
          aggregateFailed = true;
          return;
        }
  ...
  if (aggregateFailed) std::vector<std::string>{}.swap(cssFiles);
}
...
void Epub::parseCssFiles() const {
  ...
  std::array<uint64_t, epub::limits::MAX_CSS_FILES> dedupKeys{};
  if (cssFiles.size() > 1) {
    ZipFile(filepath).enumerateFileEntries([&](std::string_view entryPath, uint32_t crc32, uint32_t compressedSize) {
      ...
      for (size_t i = 0; i < cssFiles.size(); ++i) {
        if (cssFiles[i] == entryPath) {
          dedupKeys[i] = (static_cast<uint64_t>(crc32) << 32) | compressedSize;
          break;
        }
      }
    });
  }
  std::array<uint64_t, epub::limits::MAX_CSS_FILES> seenKeys{};
  ...
  for (size_t cssIndex = 0; cssIndex < cssFiles.size(); cssIndex++) {
    ...
    if (dedupKey != 0) {
      if (std::find(seenKeys.begin(), seenKeys.begin() + seenKeyCount, dedupKey) !=
          seenKeys.begin() + seenKeyCount) {
        skippedDuplicates++;
        continue;
      }
      seenKeys[seenKeyCount++] = dedupKey;
    }
    ...
    if (!cssParser->loadFromStream(tempCssFile)) {
      LOG_ERR("EBP", "CSS retained-memory/allocation limit exceeded; discarding stylesheet cache");
      tempCssFile.close();
      Storage.remove(tmpCssPath.c_str());
      cssParser->clear();
...
bool Epub::load(const bool buildIfMissing, const bool skipLoadingCss) try {
  if (!allocationValid_) return false;
  ...
  LOG_ERR("EBP", "EPUB load failed: out of memory");
  bookMetadataCache.reset();
  cssParser.reset();
  return false;
} catch (const std::length_error&) {
  LOG_ERR("EBP", "EPUB load failed: invalid allocation length");
  bookMetadataCache.reset();
  cssParser.reset();
  return false;
}
...
uint8_t* Epub::readItemContentsToBytes(const std::string& itemHref, size_t* size, const bool trailingNullByte) const {
  ...
  if (path.size() > epub::limits::MAX_HREF_BYTES || !ZipFile(filepath).getInflatedFileSize(path.c_str(), &declaredSize) ||
      declaredSize > epub::limits::MAX_IN_MEMORY_RESOURCE_BYTES) {
    LOG_ERR("EBP", "Refusing oversized in-memory EPUB resource");
    return nullptr;
  }
...
bool Epub::readItemContentsToStream(const std::string& itemHref, Print& out, const size_t chunkSize,
  ...
  if (itemHref.empty() || chunkSize == 0 || chunkSize > 64U * 1024U) {
    LOG_DBG("EBP", "Failed to read item, empty href");
    return false;
  }
  ...
  size_t declaredSize = 0;
  if (path.size() > epub::limits::MAX_HREF_BYTES || !ZipFile(filepath).getInflatedFileSize(path.c_str(), &declaredSize) ||
      declaredSize > epub::limits::MAX_EPUB_RESOURCE_BYTES) {
    LOG_ERR("EBP", "Refusing oversized streamed EPUB resource");
    return false;
  }
...
bool Epub::getItemSize(const std::string& itemHref, size_t* size) const {
  const std::string path = FsHelpers::normalisePath(itemHref);
  if (!size || path.size() > epub::limits::MAX_HREF_BYTES) return false;
