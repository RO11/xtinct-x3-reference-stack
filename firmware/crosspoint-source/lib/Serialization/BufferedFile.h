...
class BufferedFileWriter {
 public:
  ...
  void write(const void* src, const size_t len) {
    if (!okFlag || len > std::numeric_limits<size_t>::max() - pos) {
      okFlag = false;
      return;
    }
    pos += len;
...
template <typename T>
bool writePod(BufferedFileWriter& out, const T& value) {
...
inline bool writeString(BufferedFileWriter& out, const std::string& s) {
...
inline bool readString(BufferedFileReader& in, std::string& s, const size_t typeMaximum) {
  uint32_t len = 0;
  if (!readPod(in, len) || !sizedFieldFits(len, typeMaximum, in.remaining()) ||
      !detail::resizeStringChecked(s, len)) {
    s.clear();
    return false;
  }
  if (len == 0) return true;
  if (in.read(s.data(), len) != len) {
    s.clear();
    return false;
  }
  return true;
