...
#include "components/icons/inbox.h"
...
button_menu::Layout LyraTheme::getButtonMenuLayout(const GfxRenderer&, const Rect rect, const int buttonCount,
                                                    const int selectedIndex) const {
  return button_menu::pageFor(buttonCount, selectedIndex, rect.height, LyraMetrics::values.menuRowHeight,
                              LyraMetrics::values.menuSpacing);
}
...
void LyraTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = layout.first; i < layout.first + layout.count; ++i) {
    ...
    Rect tileRect = Rect{rect.x + LyraMetrics::values.contentSidePadding,
                         rect.y + layout.topInset + (i - layout.first) * layout.rowStep(), tileWidth,
