...
button_menu::Layout RoundedRaffTheme::getButtonMenuLayout(const GfxRenderer& renderer, const Rect rect,
                                                           const int buttonCount, const int selectedIndex) const {
  const int rowHeight = renderer.getLineHeight(kTitleFontId) + 20;
  return button_menu::pageFor(buttonCount, selectedIndex, rect.height, rowHeight, kSelectableRowGap);
}
...
void RoundedRaffTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = layout.first; i < layout.first + layout.count; ++i) {
    ...
    const int rowY = menuTop + layout.topInset + (i - layout.first) * rowStep;
  ...
  drawScrollBar(renderer, rect, buttonCount, layout.first, layout.capacity);
}
