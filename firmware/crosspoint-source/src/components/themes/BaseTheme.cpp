...
button_menu::Layout BaseTheme::getButtonMenuLayout(const GfxRenderer&, const Rect rect, const int buttonCount,
                                                    const int selectedIndex) const {
  return button_menu::pageFor(buttonCount, selectedIndex, rect.height, BaseMetrics::values.menuRowHeight,
                              BaseMetrics::values.menuSpacing, BaseMetrics::values.verticalSpacing);
}
...
void BaseTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = layout.first; i < layout.first + layout.count; ++i) {
    const int tileY = rect.y + layout.topInset +
                      static_cast<int>(i - layout.first) * layout.rowStep();
