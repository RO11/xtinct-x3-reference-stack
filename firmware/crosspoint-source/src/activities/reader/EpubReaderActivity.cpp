...
#include "util/ReaderOpenPolicy.h"
#include "fontIds.h"
...
EpubReaderActivity::EpubReaderActivity(GfxRenderer& renderer, MappedInputManager& mappedInput,
                                       ...
                                       const int initialRefreshCountdown)
    : Activity("EpubReader", renderer, mappedInput),
      pagesUntilFullRefresh(initialRefreshCountdown) {
  if (!epub::limits::catchAllocationFailure(
          [&]() { epub = std::shared_ptr<Epub>(std::move(ownedEpub)); })) {
    LOG_ERR("ERS", "Failed to allocate shared EPUB owner");
    allocationFailurePending = true;
  }
}

void EpubReaderActivity::handleAllocationFailure(const char* const phase) noexcept {
  LOG_ERR("ERS", "EPUB allocation failed during %s; abandoning active transaction", phase);
  if (section) section->abandonBuildNoThrow();
  section.reset();
...
void EpubReaderActivity::onEnter() try {
  ...
  if (xtinct::reader_open_policy::savedSpineNeedsReset(currentSpineIndex, spineCount)) {
    LOG_DBG("ERS", "Ignoring stale end-of-book progress: spine %d of %d", currentSpineIndex, spineCount);
    currentSpineIndex = xtinct::reader_open_policy::normalizeSavedSpine(currentSpineIndex, spineCount);
    nextPageNumber = 0;
    cachedSpineIndex = 0;
  ...
  handleAllocationFailure("reader entry");
} catch (const std::length_error&) {
  handleAllocationFailure("reader entry length check");
}
...
void EpubReaderActivity::onExit() noexcept {
  try {
    ...
    LOG_ERR("ERS", "Allocation failed during reader exit; forcing no-throw cleanup");
  } catch (const std::length_error&) {
    LOG_ERR("ERS", "Path length failure during reader exit; forcing no-throw cleanup");
  }

  // This tail is allocation-free and unconditional. In particular, never let
  // the static extractor retain a raw pointer after any earlier save/queue/path
  // operation fails, and never let a live section transaction outlive EPUB
  // ownership. abandonBuildNoThrow leaves prior committed/partial cache bytes
  // untouched.
  ...
  // Restore allocation-free in-memory UI/app state even if the optional sync
  // queue or path construction at the start of onExitImpl failed.
  renderer.setOrientation(GfxRenderer::Orientation::Portrait);
  APP_STATE.readerActivityLoadCount = 0;
  (void)epub::limits::catchAllocationFailure([&]() { APP_STATE.saveToFile(); });
}

void EpubReaderActivity::onExitImpl() {
  Activity::onExit();

  if (epub && section) {
    const float pageCount = section->estimatedTotalPages();
    const float chapterProgress =
        pageCount > 0 ? static_cast<float>(section->currentPage + 1) / pageCount : 0.0f;
    const float bookProgress =
        std::clamp(epub->calculateProgress(currentSpineIndex, chapterProgress), 0.0f, 1.0f);
    XtinctSyncClient::queueReaderProgress(
        epub->getPath(), static_cast<uint16_t>(bookProgress * 10000.0f + 0.5f));
  }
...
void EpubReaderActivity::loop() try {
  ...
  // that fired a bound function sets ignoreNextConfirmRelease so the release
  ...
  handleAllocationFailure("reader loop");
} catch (const std::length_error&) {
  handleAllocationFailure("reader loop length check");
}
...
void EpubReaderActivity::render(RenderLock&& lock) try {
...

  {
    ...
    if (!p) {
      if (::epub::pageLoadRecovery(section->lastLoadFailure()) ==
          ::epub::PageLoadRecovery::ReloadPreservedCache) {
        // The active temp build was abandoned because its shared write cursor
        // could not be proven restored. Drop only the in-memory Section and
        // reload the untouched committed/partial file; never clear that file.
        LOG_ERR("ERS", "Page load transaction abandoned - reloading preserved section cache");
        automaticPageTurnActive = false;
      ...
      if (::epub::pageLoadRecovery(section->lastLoadFailure()) ==
        ...
        // A valid finalized/partial cache must survive a transient heap race or
        // an advisory heap-gate refusal. Do not abandon, clear, reset, or queue
        // a rebuild; a later user action can retry the same committed bytes.
        LOG_ERR("ERS", "Page load resource refusal - preserving section cache");
        automaticPageTurnActive = false;
        pageLoadRetryCount = 0;
        renderer.clearScreen();
        renderer.drawCenteredText(UI_12_FONT_ID, 300, tr(STR_PAGE_LOAD_ERROR), true,
  ...
  handleAllocationFailure("reader render");
} catch (const std::length_error&) {
  handleAllocationFailure("reader render length check");
}
...
void EpubReaderActivity::renderContents(std::unique_ptr<Page> page, const int orientedMarginTop,
  ...
  // retained frame after a silent restart, leaving the old UI mixed with the
  // image.
  ...
  if (tiledGrayscale) {
    ...
    // allocates through std::string paths, so a plane buffer that "fits" but eats the render
...
void EpubReaderActivity::addBookmark() {
  ...
  if (!BookmarkFile::save(epub->getPath(), cachedBookmarks)) {
    ...
    const float boundedProgress = std::clamp(progress.percentage, 0.0f, 1.0f);
    XtinctSyncClient::queueReaderProgress(
        epub->getPath(), static_cast<uint16_t>(boundedProgress * 10000.0f + 0.5f),
        /*bookmark=*/true, bookmarkRemoved);
  }
  requestUpdate();
}
