...
void SdFirmwareUpdateActivity::onEnter() {
  Activity::onEnter();
  // Stable release marker confirms which firmware build owns the SD update flow.
  LOG_INF("FW", "SdFirmwareUpdateActivity release=%s build=%s recovery=%d", XTINCT_RELEASE_LABEL,
          XTINCT_BUILD_ID, recoveryMode ? 1 : 0);
  state = State::PICKING;
  launchPicker();
}
