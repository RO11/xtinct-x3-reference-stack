...
void SettingsActivity::rebuildSettingsLists() {
  ...
  systemSettings.push_back(SettingInfo::Action(StrId::STR_DAILY_WAKE_STATUS, SettingAction::DailyWakeStatus));
  systemSettings.push_back(SettingInfo::Action(StrId::STR_WIFI_NETWORKS, SettingAction::Network));
  systemSettings.push_back(SettingInfo::Action(StrId::STR_CLEAR_READING_CACHE, SettingAction::ClearCache));
  // XTINCT deliberately omits CrossPoint's network OTA action. That path
  // downloads unsigned firmware through an insecure HTTP client, which would
  // let a network attacker replace firmware and recover the NVS feed bearer.
  // Physical USB flashing and the Power+Up SD recovery activity remain.
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    switch (setting.action) {
      ...
      case SettingAction::TextSettings:
        ...
        break;
      case SettingAction::DailyWakeStatus:
        startActivityForResult(std::make_unique<DailyWakeStatusActivity>(renderer, mappedInput), resultHandler);
        break;
...
void SettingsActivity::render(RenderLock&&) {
  ...
  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, pageWidth, metrics.headerHeight}, tr(STR_SETTINGS_TITLE),
                 (std::string(XTINCT_RELEASE_LABEL) + " (" + XTINCT_BUILD_ID + ")").c_str());
