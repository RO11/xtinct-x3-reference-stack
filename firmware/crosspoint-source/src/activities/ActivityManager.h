...
class ActivityManager {
 ...
 public:
  ...
  void goToXtinctInbox();
  void goToDailyCards(bool scheduledWake = false, bool allowScheduledRetries = true);
  void goToPhoneSync();
  void goToWifiProvisioning(bool recoveryMode = false);
  void goToSettings();
  ...
  bool isWifiProvisioningActivity() const;
  ...
  // Trigger a render and block until it completes. Returns false instead of
  // aborting or waiting forever when the call would deadlock, the render task
  // cannot be notified, or the bounded wait expires.
